...
ArGrammarSynthesizer_DefinitenessLookupFunction::ArGrammarSynthesizer_DefinitenessLookupFunction()
    : super(LocaleUtils::ARABIC(), DEFINITENESS_VALUES)
    , possessivePronouns(ArGrammarSynthesizer::getPossessivePronouns())
{
...
SpeakableString* ArGrammarSynthesizer_DefinitenessLookupFunction::getFeatureValue(const ::inflection::dialog::DisplayValue& displayValue, const ::std::map<::inflection::dialog::SemanticFeature, ::std::u16string>& /*constraints*/) const {
    ...
    if (!displayString.empty()) {
        ...
        for (const auto& possessivePronoun : possessivePronouns) {
            if (displayString.ends_with(possessivePronoun)) {
                std::u16string_view prefix(std::u16string_view(displayString).substr(0, displayString.size() - possessivePronoun.size()));
