...
SpeakableString* SemanticConceptList::toSpeakableString(const SemanticFeature* feature) const
{
    ...
    for (auto conceptPtr : concepts) {
        ...
        if (!avoidAffixRedundancy || !morphuntion::util::StringViewUtils::startsWith(conceptResult.getPrint(), currentItemPrefix.getPrint())) {
            displayValue += currentItemPrefix;
        }
        displayValue += conceptResult;
        if (!avoidAffixRedundancy || !morphuntion::util::StringViewUtils::endsWith(conceptResult.getPrint(), currentItemSuffix.getPrint())) {
