...
Dictionary::Dictionary(const ::morphuntion::util::ULocale& locale)
    : locale(locale)
{
...
Dictionary* Dictionary::setupDictionary(const ::morphuntion::util::ULocale& locale, const ::std::string& sourcePath, const ::std::string& additionalSourcePath)
{
...
const ::morphuntion::util::ULocale& Dictionary::getLocale() const
{
    return locale;
}
...
void Dictionary::loadPartsOfSpeech(Dictionary* destination, const ::std::string& sourcePath)
{
    ...
    if (!reader) {
        throw ::morphuntion::exception::FileNotFoundException(
                morphuntion::util::StringViewUtils::to_u16string(sourcePath) + u" file not found");
    }
...
int64_t Dictionary::createGrammemeTypeIfNonExistant(::std::u16string_view type)
{
    ...
    if (existingTypeValue == typeToValue.end()) {
        ...
        if (typeValue <= 0) {
            throw ::morphuntion::exception::IllegalStateException(u"Too many properties requested for " + locale.toString() + u" type=" + std::u16string(type));
        }
...
Dictionary::extractPartsOfSpeech(::std::ifstream& in)
{
    ...
    while (in) {
        ...
        ::morphuntion::util::StringViewUtils::convert(&line, cline);

        std::u16string_view lineView(line);
        if (::morphuntion::util::StringViewUtils::startsWith(lineView, endline)) {
            break;
        }
        auto colonIdx = lineView.find(COLON_SEPARATOR);
        auto phrase(getStringSingleton(lineView.substr(0, colonIdx)));
        auto metadata = ::morphuntion::util::StringViewUtils::trim(lineView.substr(colonIdx + 1));
        int64_t combinedType = 0;
        ::std::map<::std::u16string_view, ::std::vector<::std::u16string_view>> propertyValues;
        for (::morphuntion::util::DelimitedStringIterator typeIterator(metadata, SPACE_SEPARATOR); typeIterator.hasNext(); ++typeIterator) {
        ...
        for (const auto &itr : propertyValues) {
            if (itr.second.size() > UINT8_MAX) {
                throw ::morphuntion::exception::IllegalStateException(u"Too many property values of property=" + ::std::u16string(itr.first) + u" for the phrase " + std::u16string(phrase) + u" in the locale " + locale.toString());
            }
        }
        if (!propertyValues.empty()) {
