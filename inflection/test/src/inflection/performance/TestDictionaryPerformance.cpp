...
int64_t DictionaryPerformanceInitialize(const inflection::util::ULocale& locale)
{
    auto initStart = std::chrono::high_resolution_clock::now();
    ::inflection::dictionary::DictionaryMetaData::createDictionary(locale);
    auto initEnd = std::chrono::high_resolution_clock::now();

    return (int64_t)(std::chrono::duration<double, std::milli>(initEnd - initStart).count());
}

int64_t DictionaryPerformanceGetCombinedBinaryType(const inflection::util::ULocale& locale, const std::vector<::std::u16string>& words)
{
...
int64_t DictionaryPerformanceGetPropertyValues(const inflection::util::ULocale& locale, const std::vector<::std::u16string>& words)
{
