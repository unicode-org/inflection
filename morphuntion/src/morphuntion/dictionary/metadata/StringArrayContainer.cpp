...
morphuntion::dictionary::metadata::StringArrayContainer::StringArrayContainer(::morphuntion::util::MemoryMappedFile *mappedFile)
...
morphuntion::dictionary::metadata::StringArrayContainer::StringArrayContainer(const ::std::set<::std::u16string_view>& strings, ::std::map<::std::u16string_view, int32_t>* mappingResult)
    : arraySize(int32_t(strings.size()))
    , stringIndexesWithLen(new int32_t[arraySize])
{
    if (!npc(mappingResult)->empty()) {
        throw ::morphuntion::exception::IllegalArgumentException(u"The mapping result should be empty");
    }
    ...
    for (auto string : strings) {
        ...
        if (len > LENGTH_MASK || ((offset << LENGTH_BITS) >> LENGTH_BITS) != offset) {
            throw ::morphuntion::exception::IllegalArgumentException(u"String is too large: " + std::u16string(string));
        }
...
morphuntion::dictionary::metadata::StringArrayContainer::StringArrayContainer()
    : ownData(false)
{
}
...
void morphuntion::dictionary::metadata::StringArrayContainer::write(::std::ostream& output) const
{
...
::std::u16string morphuntion::dictionary::metadata::StringArrayContainer::getString(int32_t offset) const
{
    if (offset < 0 || arraySize <= offset) {
        throw ::morphuntion::exception::IndexOutOfBoundsException(u"Offset is out of bounds");
    }
...
int32_t morphuntion::dictionary::metadata::StringArrayContainer::getIdentifierIfAvailable(::std::u16string_view string) const
{
    int32_t start = 0;
...
int32_t morphuntion::dictionary::metadata::StringArrayContainer::getIdentifier(::std::u16string_view string) const
{
...
int32_t morphuntion::dictionary::metadata::StringArrayContainer::size() const {
    return arraySize;
}
