...
using morphuntion::util::DelimitedStringIterator;
...
const ::std::map<::std::u16string_view, const char16_t*>& LOCALE_FALLBACK_PAIRS() {
    static auto LOCALE_FALLBACK_PAIRS_ = new ::std::map<::std::u16string_view, const char16_t*>(::morphuntion::resources::DataResource::getProperties(u"/org/unicode/morphuntion/inflection/pronoun.properties"));
    return *npc(LOCALE_FALLBACK_PAIRS_);
}

static std::pair<std::u16string, const char16_t *> getPronounTable(const morphuntion::util::ULocale& locale) {
    const char16_t * reader;
    auto localeItr(locale);
    const auto& localeFallbackPairs(LOCALE_FALLBACK_PAIRS());
    while ((reader = ::morphuntion::resources::DataResource::getString(u"/org/unicode/morphuntion/inflection/pronoun_" + localeItr.toString() + u".csv", true)) == nullptr
...
PronounConcept::PronounConcept(const SemanticFeatureModel& model, const ::std::vector<::morphuntion::dialog::SemanticFeatureModel_DisplayValue>& defaultDisplayData, const ::std::map<SemanticFeature, ::std::u16string>& defaultConstraints)
    : super(&model)
    , pronounData(std::make_shared<PronounData>(model, defaultDisplayData))
    , defaultConstraints(defaultConstraints)
{
    if (pronounData->numValues() == 0) {
        throw ::morphuntion::exception::IllegalArgumentException(u"Display data can not be empty.");
    }
}
    ...
    : PronounConcept(model, {}, {})
{
    ...
    if (defaultPronounIndex < 0) {
        throw ::morphuntion::exception::IllegalArgumentException(u"Unknown pronoun: " + std::u16string(initialPronoun));
    }
...
const PronounConcept::PronounEntry* PronounConcept::getFirstPossibleValue(const morphuntion::dialog::SemanticFeatureConceptBase* referencedConcept, bool returnDefault, bool matchAll) const
{
...
const PronounConcept::PronounEntry* PronounConcept::getCurrentValue(const morphuntion::dialog::SemanticFeatureConceptBase* referencedConcept, bool returnDefault, bool matchAll) const
{
...
SpeakableString* PronounConcept::toSpeakableString(const morphuntion::dialog::SemanticFeatureConceptBase& referencedConcept) const
{
