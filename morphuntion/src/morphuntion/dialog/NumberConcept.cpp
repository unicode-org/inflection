...
NumberConcept::NumberConcept(double value, const ::morphuntion::util::ULocale& language, const ::morphuntion::util::ULocale& regionLocale)
    : super()
    , language(language)
...
NumberConcept::NumberConcept(int64_t value, const ::morphuntion::util::ULocale& language, const ::morphuntion::util::ULocale& regionLocale)
    : super()
    , language(language)
    , regionLocale(regionLocale)
{
...
std::u16string NumberConcept::formatInt64(const ::icu4cxx::NumberFormat& numberFormat, int64_t value)
{
...
std::u16string NumberConcept::formatDouble(const ::icu4cxx::NumberFormat& numberFormat, double value)
{
    auto size = DEFAULT_STRING_DIGITS_SIZE;
    auto status = U_ZERO_ERROR;
    ::std::u16string result;
...
const ::morphuntion::util::ULocale& NumberConcept::getLanguage() const
{
    return language;
}

const ::morphuntion::util::ULocale& NumberConcept::getRegionLocale() const
{
    return regionLocale;
}
...
SpeakableString NumberConcept::getAsDigits() const
{
    ...
    if (::morphuntion::util::LocaleUtils::AUSTRIA().getCountry() == language.getCountry()) {
        if (::morphuntion::util::LocaleUtils::AUSTRIA() == language) {
            speakVal = format(createSimpleNumberFormat(::morphuntion::util::LocaleUtils::GERMANY()));
        }
    }
    else if (::morphuntion::util::LocaleUtils::SWITZERLAND_GERMAN().getCountry() == language.getCountry()) {
        if (::morphuntion::util::LocaleUtils::SWITZERLAND_GERMAN() == language) {
            speakVal = format(createSimpleNumberFormat(::morphuntion::util::LocaleUtils::GERMANY()));
        }
        else if (::morphuntion::util::LocaleUtils::SWITZERLAND_FRENCH() == language) {
        ...
        else if (::morphuntion::util::LocaleUtils::SWITZERLAND_ITALIAN() == language) {
            speakVal = format(createSimpleNumberFormat(::morphuntion::util::LocaleUtils::ITALY()));
        }
        else {
            speakVal = format(createSimpleNumberFormat(language));
        }
    }
    else if (::morphuntion::util::LocaleUtils::BELGIUM_FRENCH().getCountry() == language.getCountry()) {
        if (::morphuntion::util::LocaleUtils::BELGIUM_FRENCH() == language) {
            if (llabs(longValue()) < 60) {
                speakVal = format(createSimpleNumberFormat(language));
            }
            else {
...
icu4cxx::NumberFormat NumberConcept::createSimpleNumberFormat(const ::morphuntion::util::ULocale& locale) const
{
...
::std::u16string NumberConcept::format(UNumberFormatStyle type, const ::std::u16string& variant) const
{
    ...
    if (!variant.empty()) {
        ...
        catch (const morphuntion::exception::ICUException&) {
            if (::morphuntion::util::LoggerConfig::isErrorEnabled()) {
                ::morphuntion::util::Logger::error(u"Invalid variant for " + language.toString()
                       + u": ["
                       + variant
                       + u"]");
            }
...
{
    if (::morphuntion::util::StringViewUtils::startsWith(formattedValue, u".") && formattedValue.length() > 1) {
