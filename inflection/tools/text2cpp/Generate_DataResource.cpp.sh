...
echo '#include <inflection/resources/DataResource.hpp>' >> $DATA_RESOURCE_PATH
echo '#include <inflection/exception/FileNotFoundException.hpp>' >> $DATA_RESOURCE_PATH
...
echo '::std::map<::std::u16string_view, const char16_t*> inflection::resources::DataResource::getProperties(::std::u16string_view resourceName, bool returnEmptyOnFailure) {' >> $DATA_RESOURCE_PATH
...
echo '        }' >> $DATA_RESOURCE_PATH
echo '        throw ::inflection::exception::FileNotFoundException(::std::u16string(resourceName));' >> $DATA_RESOURCE_PATH
echo '    }' >> $DATA_RESOURCE_PATH
echo '    return entry->second();' >> $DATA_RESOURCE_PATH
echo '}' >> $DATA_RESOURCE_PATH
echo 'const char16_t *inflection::resources::DataResource::getString(::std::u16string_view resourceName, bool returnNullOnFailure) {' >> $DATA_RESOURCE_PATH
