...
static int32_t getValue(const std::map<::std::u16string_view, int32_t>& map, ::std::u16string_view key)
{
    ...
    if (itr == map.end()) {
        throw morphuntion::exception::IllegalArgumentException(std::u16string(key) + u" key not found");
    }
    return itr->second;
}
...
InflectionDictionary::InflectionDictionary(const ::morphuntion::util::ULocale &locale, const Inflector_InflectionDictionary &dictionary, bool addAffixPatternMappings)
    : locale(locale)
{
    ...
    numBitsForFrequencyIdx = morphuntion::dictionary::metadata::CompressedArray<int32_t>::calculateBitWidth(frequenciesSet.size());
    numBitsForPOSIdx = morphuntion::dictionary::metadata::CompressedArray<int32_t>::calculateBitWidth(grammemesSingletonsMap.size());
    numBitsForLemmaSuffixesLen = morphuntion::dictionary::metadata::CompressedArray<int32_t>::calculateBitWidth(usedBitsForLemmaSuffixesLen);
    numBitsForNumOfInflections = morphuntion::dictionary::metadata::CompressedArray<int32_t>::calculateBitWidth(usedBitsForNumOfInflections);
    ...
    inflection_Suffixes = new morphuntion::dictionary::metadata::StringContainer(inflection_SuffixesSet, &inflection_SuffixesMap);
    ...
    numBitsForSuffixIdx = morphuntion::dictionary::metadata::CompressedArray<int32_t>::calculateBitWidth(inflection_SuffixesMap.size());

    if (numBitsForGrammemesIdx + numBitsForSuffixIdx > 64) {
        throw morphuntion::exception::IllegalStateException(u"Too many bits used for inflections.");
    }

    identifierToInflectionPatternTrie = new morphuntion::dictionary::metadata::MarisaTrie<int32_t>(identifierToInflectionPatternMap);
    // -----------------------------------------------------------------------------------------------------------------------
    ...
    if (addAffixPatternMappings) {
        ...
        if (numBitsForSuffixToIdentifierRunsIdx + numBitsForSuffixToIdentifierRunsLen > 32) {
            throw morphuntion::exception::IllegalStateException(u"Too many bits used for mapping suffixes to inflection patterns.");
        }
        ...
        suffixToIdentifierRunsTrie = new morphuntion::dictionary::metadata::MarisaTrie<int32_t>(suffixToIdentifierRunsCompressedMap);
    }
    else {
        suffixToIdentifierRunsTrie = new morphuntion::dictionary::metadata::MarisaTrie<int32_t>(std::map<::std::u16string_view, int32_t>());
    }
}
...
void InflectionDictionary::write(::std::ofstream& writer, DictionaryLogger& logger) const {
    writer.write(reinterpret_cast<const char*>(&morphuntion::dictionary::Inflector_MMappedDictionary::VERSION), sizeof(morphuntion::dictionary::Inflector_MMappedDictionary::VERSION));
    writer.write(reinterpret_cast<const char*>(&morphuntion::dictionary::Inflector_MMappedDictionary::ENDIANNESS_MARKER), sizeof(morphuntion::dictionary::Inflector_MMappedDictionary::ENDIANNESS_MARKER));
    // Write reserved options. It's currently unused.
    writer.write(reinterpret_cast<const char*>(&morphuntion::dictionary::Inflector_MMappedDictionary::OPTIONS), sizeof(morphuntion::dictionary::Inflector_MMappedDictionary::OPTIONS));
    ...
    morphuntion::dictionary::metadata::CompressedArray compressedInflectionsArray(inflectionsArray);
    ...
    morphuntion::dictionary::metadata::CompressedArray compressedSuffixesToInflectionsArray(suffixesToInflections.getSingletons());
...
Inflector_InflectionDictionary* InflectionDictionary::getInflectionPatterns(const ::morphuntion::util::ULocale& locale, xmlDocPtr xmlDoc,
                                                                            const std::map<::std::u16string_view, int32_t>& grammemeStringIndices)
{
    ...
    if (patterns == nullptr) {
        throw ::morphuntion::exception::XMLParseException(u"Incorrect root element -- expecting <patterns>, got <null>");
    }
    if (xmlStrEqual(patterns->name, (const xmlChar *) "patterns") == 0) {
        throw ::morphuntion::exception::XMLParseException(u"Incorrect root element -- expecting <patterns>, got <" + ::morphuntion::util::StringUtils::to_u16string((const char *)patterns->name) + u">");
    }
    ...
    while (child != nullptr) {
        ...
        if (xmlStrEqual(cur->name, (const xmlChar *) "pattern") == 0) {
            throw ::morphuntion::exception::XMLParseException(u"Expecting element <pattern>, got <" + ::morphuntion::util::StringUtils::to_u16string(std::string(reinterpret_cast<const char*>(patterns->name))) + u">");
        }
        xmlChar* patternName = xmlGetProp(cur, (const xmlChar*) "name");
        if (patternName == nullptr) {
            throw ::morphuntion::exception::XMLParseException(u"Attribute \"name\" not specified in <pattern> element");
        }
        std::u16string identifier16;
        ::morphuntion::util::StringUtils::convert(&identifier16, avalue8 = reinterpret_cast<const char*>(patternName));
        xmlFree(patternName);
        ...
        if (!insertionResult.second) {
            throw ::morphuntion::exception::XMLParseException(u"Pattern name " + identifier16 + u" is not unique");
        }
        ...
        while (childChild != nullptr) {
            ...
            if (xmlStrEqual(curChild->name, (const xmlChar *) "pos") != 0) {
                ...
                ::morphuntion::util::StringUtils::convert(&avalue16, avalue8 = reinterpret_cast<const char*>(pos));
                inflectPattern.partsOfSpeech |= morphuntion::dictionary::metadata::StringContainer::convertIdentifierToBit(getValue(grammemeStringIndices, avalue16));
                xmlFree(pos);
            }
                ...
                if (newSuffix != nullptr) {
                    ::morphuntion::util::StringUtils::convert(&avalue16, avalue8 = reinterpret_cast<const char*>(newSuffix));
                    xmlFree(newSuffix);
                }
                ...
                while (childChildChild != nullptr) {
                    ...
                    if (xmlStrEqual(curChildChild->name, (const xmlChar *) "inflection") != 0) {
                        ...
                        for (xmlAttrPtr attr = curChildChild->properties; nullptr != attr; attr = attr->next) {
                            ...
                            ::morphuntion::util::StringUtils::convert(&avalue16, avalue8 = reinterpret_cast<const char*>(value));
                            xmlFree(value);
                            grammemes |= morphuntion::dictionary::metadata::StringContainer::convertIdentifierToBit(getValue(grammemeStringIndices, avalue16));
                        }
                        xmlChar *inflectionText = xmlNodeGetContent(curChildChild);
                        ::morphuntion::util::StringUtils::convert(&avalue16, reinterpret_cast<const char *>(inflectionText));
...
Inflector_InflectionDictionary* InflectionDictionary::readXML(const ::morphuntion::util::ULocale& locale, const std::string& source,
                                                              const std::map<::std::u16string_view, int32_t>& grammemeStringIndices)
{
    ...
    {
        ::morphuntion::util::Finally finally([&xmlDoc]() noexcept {
            if (xmlDoc != nullptr) {
                xmlFreeDoc(xmlDoc);
            }
            xmlCleanupParser();
        });
        ::std::u16string resourceName = morphuntion::util::StringUtils::to_u16string(source);
        ::morphuntion::util::MemoryMappedFile mMapFile(resourceName);
        xmlDoc = xmlParseMemory(mMapFile.getData(), int32_t(mMapFile.getSize()));
        if (xmlDoc == nullptr) {
            throw ::morphuntion::exception::IOException(resourceName);
        }
