...
Tokenizer* TokenizerFactory::createTokenizer(const util::ULocale& locale)
{
    ...
    if (tokenizer == nullptr) {
        ...
        if (config.empty()) {
            ...
            if (!readConfig(util::StringViewUtils::to_u16string(util::LocaleConstants::ENGLISH), &config)) {
...
Tokenizer* TokenizerFactory::createTokenizerObject(const util::ULocale& locale, const ::std::map<::std::u16string_view, const char16_t*>& systemConfig)
{
    ...
    if (inClassNameEntry != systemConfig.end()) {
        std::u16string_view inClassName = inClassNameEntry->second;
        ...
        auto *entry = inflection::util::ArrayUtils::searchSorted<TOKENIZER_CONSTRUCTORS>(inClassName,
            [](const auto& item) { return item.className; });
        if (entry != nullptr) {
            ...
            auto tokenizer = new Tokenizer(entry->construct(locale, systemConfig));
