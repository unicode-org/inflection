...
DictionaryMetaData_MMappedDictionary::DictionaryMetaData_MMappedDictionary(const morphuntion::util::ULocale& locale)
    : locale(locale)
{
}

DictionaryMetaData_MMappedDictionary::DictionaryMetaData_MMappedDictionary(::morphuntion::util::MemoryMappedFile* memoryMappedRegion, const ::std::u16string& sourcePath)
    ...
    , inflectionKeyIdentifier(propertyNameToKeyId.getIdentifierIfAvailable(morphuntion::dictionary::Inflector_MMappedDictionary::INFLECTION_KEY))
    , bitsPropertyValueMapKeyMask((int32_t(1) << bitsPropertyValueMapKey) - 1)
    , bitsPropertyValueMapKeySize(morphuntion::dictionary::metadata::CompressedArray<int32_t>::calculateBitWidth(propertyNameToKeyId.size()))
...
const ::morphuntion::util::ULocale& DictionaryMetaData_MMappedDictionary::getLocale() const
{
    return locale;
}
...
std::optional<int64_t> DictionaryMetaData_MMappedDictionary::getWordType(std::u16string_view word) const
{
    ...
    if (result) {
        ...
        auto typeSingleton = morphuntion::dictionary::metadata::CompressedArray<int16_t>::extractValue(dataSingleton, 0, bitsTypesSingletons);
        if (typeSingleton != 0) {
...
std::optional<int64_t> DictionaryMetaData_MMappedDictionary::getValueOfType(std::u16string_view type) const
{
    ...
    if (id >= 0) {
        return morphuntion::dictionary::metadata::StringContainer::convertIdentifierToBit(id);
    }
    return {};
}
...
{
    auto id = morphuntion::dictionary::metadata::StringContainer::convertBitToIdentifier(value);
...
bool DictionaryMetaData_MMappedDictionary::getWordPropertyInternalIdentifiers(std::vector<int32_t> &propertyIdentifiers, std::u16string_view word, int32_t propertyNameIdentifier) const {
    ...
    if (trieResult) {
        ...
        auto offset = morphuntion::dictionary::metadata::CompressedArray<int32_t>::extractValue(dataSingleton, bitsTypesSingletons, bitsPropertyMapId);
        if (offset != 0) {
            propertyIdentifiers.clear();

            const auto firstValue = propertyValueMaps.read(offset);
            int32_t cumulativeOffset = 0;
            int32_t originalMapSize = morphuntion::dictionary::metadata::CompressedArray<int32_t>::extractValue(firstValue, bitsPropertyValueMapKey + bitsPropertyValueMapValuesSize, bitsPropertyValueMapKeySize);
            for (int32_t mapIdx = 0; mapIdx < originalMapSize; mapIdx++) {
                uint32_t keyEntry = morphuntion::dictionary::metadata::CompressedArray<int32_t>::extractValue(propertyValueMaps.read(offset + mapIdx), 0, bitsPropertyValueMapKey + bitsPropertyValueMapValuesSize);
...
DictionaryMetaData_MMappedDictionary* DictionaryMetaData_MMappedDictionary::createDictionary(const ::std::u16string& sourcePath)
{
    ...
    ::std::unique_ptr<::morphuntion::util::MemoryMappedFile> mappedFile(new ::morphuntion::util::MemoryMappedFile(sourcePath));
    auto mappedRegion = mappedFile.get();

    if (mappedRegion->getSize() < sizeof(MAGIC_MARKER) + sizeof(VERSION) + sizeof(ENDIANNESS_MARKER)) {
        throw ::morphuntion::exception::IOException( u"Input file " + sourcePath + u" is too small");
    }
    ...
    if (strncmp(magicMarker, MAGIC_MARKER, sizeof(MAGIC_MARKER)) != 0) {
        throw ::morphuntion::exception::IOException(u"Input file " + sourcePath + u" has invalid header");
    }

    auto version = npc(mappedRegion)->read<int64_t>();
    if (version != VERSION) {
        throw ::morphuntion::exception::IncompatibleVersionException(u"Input file " + sourcePath + u" has an incompatible version");
    }

    auto endiannessMarker = npc(mappedRegion)->read<int16_t>();
    if (endiannessMarker != ENDIANNESS_MARKER) {
        throw ::morphuntion::exception::IOException(u"Input file " + sourcePath + u" was built for a different architecture");
    }
