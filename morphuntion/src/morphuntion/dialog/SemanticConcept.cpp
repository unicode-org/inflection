    ...
    , defaultToSemantic(defaultToSemantic)
{
    if (!defaultToSemantic && (displayData == nullptr || npc(displayData)->getValues().empty())) {
        throw ::morphuntion::exception::IllegalArgumentException(u"Display data is missing for semantic value \"" + semantic.toString() + u"\" and it can not be displayed by default");
    }
}
        ...
        if (defaultDisplayFunction != nullptr) {
            ...
            if (displayData == nullptr) {
                if (defaultToSemantic) {
                    ...
                    throw ::morphuntion::exception::IllegalStateException(u"Display data is missing for \"" + semantic.toString() + u"\" and it can not be displayed by default");
                }
            }
            else {
