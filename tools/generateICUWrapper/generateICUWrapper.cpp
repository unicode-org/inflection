...
struct Function {
    ...
    ::std::string getErrorHandlingTrailer() const {
        if (hasErrorCodeArg()) {
            return "    ::inflection::exception::ICUException::throwOnFailure(ec);\n";
        }
        return "";
    }
};
...
//            std::cerr << "Skipping unknown prefix: " << prefix << "\n";
    ...
    dest << "#include <inflection/exception/ICUException.hpp>\n";
    dest << "#include <inflection/exception/NullPointerException.hpp>\n";
    ...
    for (const auto &cls : declartionsForClass) {
        ...
        if (hasWrittenCopyCtorForClass.find(clsName) == hasWrittenCopyCtorForClass.end()) {
            ...
            dest << "        throw ::inflection::exception::NullPointerException();\n";
            dest << "    }\n";
