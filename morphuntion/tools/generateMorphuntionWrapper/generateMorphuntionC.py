...
HEADER_TEMPLATE = """#include <morphuntion/{dir_}{class_}.h>

#include <morphuntion/{dir_}{class_}.hpp>
...
def cast_to_c_type(modifier: str, c_type: str) -> str:
    if c_type == 'CFStringRef':
        return 'morphuntion::util::CFUtils::to_CFString'
    elif c_type == 'CFLocaleRef':
        return 'morphuntion::util::CFUtils::convert'
...
def remap_type(c_type: str):
    c_type = re.sub("^MDP", "morphuntion::dialog::proxy::", c_type)
    c_type = re.sub("^MD", "morphuntion::dialog::", c_type)
    c_type = re.sub("^MDICT", "morphuntion::dictionary::", c_type)
    c_type = re.sub("^MR", "morphuntion::resources::", c_type)
    c_type = re.sub("^MANA", "morphuntion::analysis::", c_type)
    c_type = re.sub("^MFEAT", "morphuntion::lang::features::", c_type)
    c_type = re.sub("^MLANG", "morphuntion::lang::", c_type)
    return c_type


def cast_to_cpp_type(c_type: str, arg: str) -> str:
    if c_type == 'CFStringRef':
        return f'morphuntion::util::CFUtils::to_u16string({arg})'
    elif c_type == 'CFLocaleRef':
        return f'morphuntion::util::CFUtils::convert({arg})'
    elif c_type.startswith('M'):
...
def convert_to_c(fpath: Path):
    ...
    dir_ = "" if dir_name == 'morphuntion' else dir_name + "::"
    class_name = fname.rstrip(".h")
    class_ = "::morphuntion::" + dir_ + class_name

    data = fpath.read_text()
    data = re.sub(r"#pragma.+?\n", "", data)
    data = re.sub(r"#include.+?\n", "", data)
    data = HEADER_TEMPLATE.format(dir_=dir_name + "/" if dir_name != "morphuntion" else "", class_=class_name) + data
