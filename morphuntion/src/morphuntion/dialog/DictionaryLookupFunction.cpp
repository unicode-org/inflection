...
DictionaryLookupFunction::DictionaryLookupFunction(const ::morphuntion::util::ULocale& locale, const ::std::vector<::std::u16string>& tags)
    : super()
    , dictionaryMorphology(locale)
    , tokenizer(npc(morphuntion::tokenizer::TokenizerFactory::createTokenizer(locale)))
{
    ::morphuntion::util::Validate::notNull(dictionaryMorphology.getDictionary().getBinaryProperties(&mask, tags));
}
...
::std::u16string DictionaryLookupFunction::determinePhrase(const ::std::u16string& word, bool firstWordDeterminesValue) const
{
    ...
    if (countMaskedEnabledBits(properties) != 1) {
        ...
        if (properties == 0) {
            ...
            ::std::unique_ptr<::morphuntion::tokenizer::TokenChain> tokenChain(npc(tokenizer->createTokenChain(word)));
            if (tokenChain->getWordCount() > 1) {
                // It's an unknown phrase.
                const ::morphuntion::tokenizer::Token_Word *firstWordToken = nullptr;
                const ::morphuntion::tokenizer::Token_Word *firstRelevantToken = nullptr;
...
void DictionaryLookupFunction::getImportantTokens(const ::morphuntion::tokenizer::TokenChain *tokenChain,
                                                  bool firstWordDeterminesValue,
                                                  const morphuntion::tokenizer::Token_Word *& firstWordToken,
                                                  const morphuntion::tokenizer::Token_Word *& firstRelevantToken) const
{
    int64_t properties = 0;
    for (const auto &token: *npc(tokenChain)) {
        if (dynamic_cast< const morphuntion::tokenizer::Token_Word * >(&token) != nullptr) {
            if (firstWordToken == nullptr) {
                firstWordToken = dynamic_cast< const morphuntion::tokenizer::Token_Word * >(&token);
            }
            if (getDictionary().getCombinedBinaryType(&properties, token.getCleanValue()) != nullptr
                && (properties & mask) != 0)
            {
                firstRelevantToken = dynamic_cast< const morphuntion::tokenizer::Token_Word * >(&token);
