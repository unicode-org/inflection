...
template <typename T>
class morphuntion::dictionary::metadata::CompressedArray {
public:
    ...
    explicit CompressedArray(::morphuntion::util::MemoryMappedFile* mappedFile);
...
template<typename T>
morphuntion::dictionary::metadata::CompressedArray<T>::CompressedArray(::morphuntion::util::MemoryMappedFile* mappedFile)
    : ownData(false)
{
...
template<typename T>
morphuntion::dictionary::metadata::CompressedArray<T>::CompressedArray(int32_t wordWidth, int32_t arraySize)
...
template<typename T>
int8_t morphuntion::dictionary::metadata::CompressedArray<T>::calculateBitWidth(uint64_t allUsedBits)
{
    int8_t wordWidth = 0;
    while (allUsedBits != 0) {
        wordWidth++;
...
template<typename T>
int8_t morphuntion::dictionary::metadata::CompressedArray<T>::calculateBitWidth(const ::std::vector<T>& input)
{
...
template<typename T>
T morphuntion::dictionary::metadata::CompressedArray<T>::extractValue(uint64_t value, uint8_t start, uint8_t len)
{
    if ((start + len) > 64) {
...
template<typename T>
morphuntion::dictionary::metadata::CompressedArray<T>::CompressedArray(const ::std::vector<T>& input)
...
template<typename T>
morphuntion::dictionary::metadata::CompressedArray<T>::~CompressedArray() {
    if (ownData) {
        delete[] data;
    }
}
...
template <typename T>
bool morphuntion::dictionary::metadata::CompressedArray<T>::isEmpty() const
{
    return dataArrayLength == 0;
}

template <typename T>
T morphuntion::dictionary::metadata::CompressedArray<T>::read(int32_t index) const{
    ...
    if (index < 0 || endData > data + dataArrayLength) {
        throw morphuntion::exception::IndexOutOfBoundsException(u"Invalid CompressedArray index");
    }
...
template <typename T>
void morphuntion::dictionary::metadata::CompressedArray<T>::write(int32_t index, T value) {
    ...
    if (endData > data + dataArrayLength) {
        throw morphuntion::exception::IndexOutOfBoundsException(u"CompressedArray index too big");
    }
...
template <typename T>
void morphuntion::dictionary::metadata::CompressedArray<T>::serialize(::std::ostream& writer) const
{
