...
{
    if (error != nullptr) {
        AutoCFRelease<CFStringRef> message(CFStringCreateWithCString(nullptr, e.what(), kCFStringEncodingUTF8));
        *error = CFErrorCreate(nullptr, message.value, 0, nullptr);
    }
}
...
{
    AutoCFRelease<CFStringRef> localeName(CFUtils::to_CFString(locale.getName()));
    CFLocaleRef localeRef = CFLocaleCreate(nullptr, localeName.value);
    return localeRef;
}
