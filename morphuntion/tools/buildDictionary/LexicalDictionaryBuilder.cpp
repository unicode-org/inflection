...
using morphuntion::dictionary::DictionaryMetaData_MMappedDictionary;
using morphuntion::dictionary::metadata::CompressedArray;
using morphuntion::dictionary::metadata::MarisaTrie;
using morphuntion::dictionary::metadata::StringArrayContainer;
using morphuntion::dictionary::metadata::StringContainer;
...
inline void LexicalDictionaryBuilder::writeValue(uint64_t &valueBase, int32_t start, int32_t len, T valueToWrite) {
    if (!(0 <= start && (start+len) < 64)) {
        throw morphuntion::exception::IndexOutOfBoundsException(u"Illegal start or length value");
    }
...
void LexicalDictionaryBuilder::write(::std::ofstream& writer,
                                     ...
                                     const morphuntion::dictionary::metadata::MarisaTrie<uint64_t>& wordsToDataTrie,
                                     const std::vector<int64_t>& typeSingletons,
                                     const morphuntion::dictionary::metadata::CompressedArray<uint64_t>& wordsToDataSingletons,
                                     const morphuntion::dictionary::metadata::StringArrayContainer& propertyNameToKeyId,
                                     const morphuntion::dictionary::metadata::StringContainer& propertyValuesStringContainer,
                                     const morphuntion::dictionary::metadata::CompressedArray<int32_t>& propertyValueMaps,
                                     const morphuntion::dictionary::metadata::StringArrayContainer& typesStringContainer,
                                     bool hasInflectionTable)
{
    ...
    if (language.length() >= DictionaryMetaData_MMappedDictionary::MAX_LANGUAGE_CODE_LENGTH) {
        throw ::morphuntion::exception::IOException(u"The language code of this metadata is too long for serialization.");
    }
        ...
        const ::std::map<int64_t, ::std::u16string_view>& valueToType)
{
    ...
    for (const auto& [value, type] : valueToType) {
        ...
        if (indexItr == typeStringIndices.end()) {
            throw morphuntion::exception::IllegalArgumentException(u"string not found in typeStringIndices");
        }
        auto identifierBit = StringContainer::convertIdentifierToBit(indexItr->second);
        if (morphuntion::dictionary::metadata::StringContainer::convertBitToIdentifier(identifierBit) != indexItr->second) {
            throw morphuntion::exception::IllegalArgumentException(u"bit did not roundtrip");
        }
...
static int64_t mapValueToContainerBits(int64_t binaryProperties, std::map<int64_t, int64_t> bitmap)
{
    ...
    while (binaryProperties != 0) {
        ...
        if (newBitItr == bitmap.end()) {
            throw morphuntion::exception::IllegalArgumentException(u"bit not found in bitmap");
        }
        ...
        std::vector<int32_t>& propertyValueMaps)
{
    ...
    for (const auto& [word, maps] : data) {
        ...
        for (const auto& [key, values] : maps) {
            ...
            if (inflectionDictionary == nullptr || key != ::morphuntion::dictionary::Inflector_MMappedDictionary::INFLECTION_KEY) {
    ...
    if ((bitsKeysSize + bitsPropertyMapValuesSize + bitsPropertyMapKeyId) > 32) {
        throw ::morphuntion::exception::IllegalStateException(u"Too many property keys");
    }
    ...
    auto inflectionKeyIdIter(keyContainerMap.find(morphuntion::dictionary::Inflector_MMappedDictionary::INFLECTION_KEY));
    ...
    for (const auto& [word, wordEntry] : data) {
        for (const auto& [propertyName, properties] : wordEntry) {
            ...
            if (propertyKeyIdIter == keyContainerMap.end()) {
                throw ::morphuntion::exception::IllegalStateException(u"Key not found: " + std::u16string(propertyName));
            }
            ...
            for (const auto& property : properties) {
                ...
                if (strId == containerLookupMap.end()) {
                    throw ::morphuntion::exception::IllegalStateException(u"PropertyValueMap: Could not find string " + ::std::u16string(property) + u" in string container.");
                }
        ...
        if (vectorData.empty()) {
            throw ::morphuntion::exception::IllegalStateException(u"PropertyValueMap: Map for " + ::std::u16string(word) + u" can not be empty.");
        }
                                               ...
                                               bool addAffixPatternMappings)
{
    ...
    if (typeSingletons.size() >= INT16_MAX) {
        throw morphuntion::exception::IllegalArgumentException(u"Too many type singletons");
    }
    if (logger.isVerbose() && !typeStrings.empty()) {
        for (uint64_t bit = (uint64_t(1) << (typeStrings.size() - 1)); bit; bit >>= 1) {
            if ((bit & usedProperties) == 0) {
                logger.log(dictionary.getLocale().getName() + " " + morphuntion::util::StringViewUtils::to_string(dictionary.getValueToType().find(bit)->second) + " is unreferenced in the dictionary");
            }
        }
    }
    ...
    morphuntion::dictionary::metadata::CompressedArray propertyValueMaps(propertyValueMapsVector);
    ...
    int8_t numBitsWordsToDataTrieArray = morphuntion::dictionary::metadata::CompressedArray<int64_t>::calculateBitWidth(wordsToDataTrieBitMask);
    int8_t numBitsDataSingletonsMapToIndexArray = morphuntion::dictionary::metadata::CompressedArray<int64_t>::calculateBitWidth(dataSingletons.size());
