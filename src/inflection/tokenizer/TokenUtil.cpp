...
{
    static auto DELIMITER_CHARACTERS_ = ::inflection::util::UnicodeSetUtils::freeze(new ::icu4cxx::UnicodeSet(u"[\\p{Separator}\\p{Control}\\p{Word_Break=format}'’·・]"));
    return *npc(DELIMITER_CHARACTERS_);
}

const ::icu4cxx::UnicodeSet &TokenUtil::SIGNIFICANT_CHARACTERS()
{
    static auto SIGNIFICANT_CHARACTERS_ = ::inflection::util::UnicodeSetUtils::freeze(new ::icu4cxx::UnicodeSet(u"[\\&\\p{Letter}\\p{Number}\\p{Symbol}]"));
    return *npc(SIGNIFICANT_CHARACTERS_);
}

const ::icu4cxx::UnicodeSet &TokenUtil::PRIOR_TO_DASH_DELIMITER()
{
    static auto PRIOR_TO_DASH_DELIMITER_ = ::inflection::util::UnicodeSetUtils::freeze(new ::icu4cxx::UnicodeSet(u"[[[:Letter:]-[:Common:]-[:Thai:]-[:Hiragana:]-[:Katakana:]-[:Khmer:]][[:Nonspacing_Mark:]&[:Zinh:]]]"));
