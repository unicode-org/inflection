...
SemiticWordAndDelimiterTokenExtractor::SemiticWordAndDelimiterTokenExtractor(const ::morphuntion::util::ULocale& locale, const ::icu4cxx::UnicodeSet& nativeSet, const ::std::map<::std::u16string_view, const char16_t*>& config)
    : super(locale, config)
    , nativeSet(nativeSet)
{
...
morphuntion::tokenizer::iterator::TokenExtractorIterator* SemiticWordAndDelimiterTokenExtractor::createIterator(std::u16string_view str) const
{
    return new morphuntion::tokenizer::locale::SemiticTokenExtractorIterator(*this, str);
}
...
SemiticWordAndDelimiterTokenExtractor::decompoundWord(std::vector<int32_t>* boundaries, std::u16string_view charSequence, int32_t start, int32_t end) const
{
    ...
    if (originalLength <= MINIMUM_WORD_LENGTH || !::morphuntion::util::UnicodeSetUtils::containsSome(nativeSet, compoundWord) || isKnownWord(compoundWord)) {
        return;
    }
    int32_t prefixIdx;
...
{
    if (!toNormalize.empty() && ::morphuntion::util::StringViewUtils::endsWith(src, toNormalize)
...
{
    int32_t number = 0;
    auto prefixes = config.find(::std::u16string(PREFIX_KEY) + u"." + ::morphuntion::util::StringUtils::to_u16string(number));
    while (prefixes != config.end()) {
        number++;
        prefixes = config.find(::std::u16string(PREFIX_KEY) + u"." + ::morphuntion::util::StringUtils::to_u16string(number));
    }
    return number;
}
...
void SemiticWordAndDelimiterTokenExtractor::addPrefix(SemiticWordAndDelimiterTokenExtractor_AffixRecord* tree, ::std::u16string_view affix)
{
    ...
    morphuntion::util::Validate::isFalse(affix.empty());
...
void SemiticWordAndDelimiterTokenExtractor::addPrefixes(const ::std::map<::std::u16string_view, const char16_t*>& config)
{
    ...
    int32_t idx = 0;
    auto prefixes = config.find(::std::u16string(PREFIX_KEY) + u"." + ::morphuntion::util::StringUtils::to_u16string(idx));
    while (prefixes != config.end()) {
        ::std::u16string prefixesString;
        ::morphuntion::util::StringViewUtils::lowercase(&prefixesString, prefixes->second, getLocale());
        prefixTree[idx] = SemiticWordAndDelimiterTokenExtractor_AffixRecord();
        for (::morphuntion::util::DelimitedStringIterator iterator(prefixesString, commaSeparator); iterator.hasNext(); ++iterator) {
            addPrefix(&prefixTree[idx], *iterator);
        }
        idx++;
        prefixes = config.find(::std::u16string(PREFIX_KEY) + u"." + ::morphuntion::util::StringUtils::to_u16string(idx));
    }
}
...
void SemiticWordAndDelimiterTokenExtractor::addSuffixes(const ::std::map<::std::u16string_view, const char16_t*>& config)
{
    ...
    if (suffixes != config.end()) {
        ...
        ::morphuntion::util::StringViewUtils::lowercase(&suffixesString, suffixes->second, getLocale());
        for (::morphuntion::util::DelimitedStringIterator iterator(suffixesString, commaSeparator); iterator.hasNext(); ++iterator) {
...
void SemiticWordAndDelimiterTokenExtractor::addSuffix(SemiticWordAndDelimiterTokenExtractor_AffixRecord* tree, ::std::u16string_view affix)
{
    ...
    addPrefix(tree, *::morphuntion::util::StringUtils::reverse(&affixCopy));
}
