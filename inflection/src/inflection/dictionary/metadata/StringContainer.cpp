...
StringContainer::StringContainer(::inflection::util::MemoryMappedFile *mappedFile)
...
{
    inflection::util::Validate::isTrue(npc(mappingResult)->empty(), u"The mapping result should be empty");
    ...
    trie.build(keyset);
    inflection::util::Validate::isTrue(((int32_t) trie.num_keys()) == _size, u"Number of keys in string container should match with size of data");
